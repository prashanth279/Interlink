...
async function processAccount(acc, idx) {
    ...
    if (exp > 0 && nowTs > exp) {
        ...
        displayAccount(acc, idx, nameStr, accLog, prevLog);
        return;
    }
    ...
    try {
        ...
        if (check.data?.data?.isClaimable) {
            ...
            accLog.windows[winHour] = moment().format('HH:mm'); // Saves local time
...
// --- DISPLAY LOGIC (THEME RESTORED) ---
function displayAccount(acc, idx, nameStr, accLog, prevLog) {
    const id = acc.email || acc.deviceId;
    ...
    console.log(`${c.cy}⫸ ${c.wh}${c.b}Acc ${idx + 1}:${c.rst} ${c.p}${nameStr}${c.rst} | ${pStat}`);
    console.log(`${c.cy}⸽ ${c.rst}Status: ${stat}`);
    console.log(`${c.cy}⸽ ${c.rst}G: ${c.w}${accLog.tokens.G.toFixed(2)}${c.rst} | S: ${c.wh}${accLog.tokens.S.toFixed(2)}${c.rst} | D: ${c.p}${accLog.tokens.D.toFixed(2)}${c.rst} | INT: ${c.g}${accLog.tokens.INT.toFixed(2)}${c.rst}`);
    
    let ystStr = prevLog ? prevLog.tokens.G.toFixed(2) : '0.00';
    console.log(`${c.cy}⸽ ${c.rst}Profit: ${c.g}+${dailyProfit}${c.rst} ${c.gr}(${accLog.lastSync || '--'})${c.rst} | YST: ${c.wh}${ystStr}${c.rst}`);
    
    // Multi-line Local Window Grid
    ...
    windowPairs.forEach(pair => {
        const line = pair.map(h => {
            ...
            return isPast ? `${c.e}${localWin}${c.rst}` : `${c.gr}${localWin}${c.rst}`;
        }).join(` ${c.gr}|${c.rst} `);
...
async function main() {
    ...
    accounts.forEach(acc => {
        ...
        if (!forecasts[id]) {
            ...
            if (logs[today]?.[id]?.windows?.[winK]) {
                // Current window is completed, schedule next
                const nW = getNextWindow();
                forecasts[id] = moment.utc(nW).add(Math.floor(Math.random() * 15) + 5, 'minutes');
            } else {
                // Missed current window, claim immediately (in 2 seconds)
    ...
    while (!isShuttingDown) {
        ...
        // Print Original Header Style
        console.log(`\n           ${c.s}${c.b}INTERLINK FARMER: STEALTH EDITION${c.rst}`);
